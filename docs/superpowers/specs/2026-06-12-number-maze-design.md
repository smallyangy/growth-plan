...
**双色模式（5×5 dual）的点击顺序**（明确化以避免歧义）：
- 红色路径：1R, 2R, 3R, ..., 12R（12 个数字）
- 蓝色路径：1B, 2B, 3B, ..., 13B（13 个数字）
- 期望顺序：**交替**——1R → 1B → 2R → 2B → 3R → 3B → ... → 12R → 12B → 13B
- 即：每点完一个红色数字，紧接着点对应的蓝色数字
- 这种交替方式训练"任务切换"能力（与报告附录 A2 设计的"颜色编码"一致）
